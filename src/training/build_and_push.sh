...
#
# Uso:
#   bash src/training/build_and_push.sh <image-name>
#
# Parámetros:
...
# [CAMBIO APLICADO]: Apuntar a la carpeta src/training en lugar de decision_trees
chmod +x src/training/train
chmod +x src/training/serve
...
# [CAMBIO APLICADO]: Apuntar al Dockerfile específico y usar el punto (.) para el contexto raíz
docker build --network sagemaker -t ${image} -f src/training/Dockerfile .
