...
def test_generar_grid_base_columnas_correctas(df_ventas_grid):
    """Prueba que se generen las columnas correctas en la matriz base"""
...
def test_generar_grid_base_crea_producto_cartesiano(df_ventas_grid):
    """Prueba que se generen todas las combinaciones de mes-tienda-item"""
    resultado = generar_grid_base(df_ventas_grid)
...
def test_agregar_historia_crea_nueva_columna(df_ventas_historia):
    """Prueba que se cree la nueva columna de historia con el nombre correcto"""
    resultado = agregar_historia(
        df_ventas_historia, meses_atras=[1], columna_base="ventas"
    )
    assert "ventas_mes_ant_1" in resultado.columns


def test_agregar_historia_desplaza_valores(df_ventas_historia):
    """Prueba que los valores se desplacen correctamente para el mes anterior"""
    resultado = agregar_historia(
        df_ventas_historia, meses_atras=[1], columna_base="ventas"
    )
    # Extraemos el valor del mes anterior para el registro del mes 1
    venta_pasada = resultado.loc[
        resultado["date_block_num"] == 1, "ventas_mes_ant_1"
    ].values[0]
    assert venta_pasada == 100.0


def test_agregar_historia_multiples_meses(df_ventas_historia):
    """Prueba que se puedan agregar múltiples columnas de historia para diferentes meses atrás"""
    resultado = agregar_historia(
        df_ventas_historia, meses_atras=[1, 2], columna_base="ventas"
    )
    assert "ventas_mes_ant_1" in resultado.columns
    assert "ventas_mes_ant_2" in resultado.columns
    venta_pasada_2 = resultado.loc[
        resultado["date_block_num"] == 2, "ventas_mes_ant_2"
    ].values[0]
